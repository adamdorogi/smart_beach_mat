...
class Account {
    ...
    public function __construct($connection) {
        ...
        $token = str_replace('Bearer ', '', $_SERVER['HTTP_AUTHORIZATION']); // Extract Bearer token from Authorization header.
    ...
    private function read($token) {
        ...
        $statement = $this->connection->prepare('SELECT BIN_TO_UUID(`id`) AS `id`, `email`, `is_verified`, `created_on` FROM `smart_beachmat_account` WHERE `id` IN (SELECT `account_id` FROM `smart_beachmat_token` WHERE `token` = :token)');
    ...
    private function update_email($token, $attributes) {
        ...
        $statement = $this->connection->prepare('SELECT `id`, `password` FROM `smart_beachmat_account` WHERE `id` IN (SELECT `account_id` FROM `smart_beachmat_token` WHERE `token` = :token)');
        $statement->bindParam(':token', $token);
        $statement->execute();
    ...
    private function update_password($token, $attributes) {
        ...
        $statement = $this->connection->prepare('DELETE FROM `smart_beachmat_token` WHERE `account_id` = :id AND `token` <> :token');
        $statement->bindParam(':token', $token);
        $statement->bindParam(':id', $account_id);
        $statement->execute();
    ...
    private function delete($token) {
        $statement = $this->connection->prepare('DELETE FROM `smart_beachmat_account` WHERE `id` IN (SELECT `account_id` FROM `smart_beachmat_token` WHERE `token` = :token)');
        $statement->bindParam(':token', $token);
        $statement->execute();
